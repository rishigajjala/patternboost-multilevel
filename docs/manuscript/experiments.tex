...
hardness.  Epsilon-net and representability-separation experiments are treated
as separate exploratory appendix tasks, not as rows in the main three-problem
table.
\end{abstract}
...
This experiment should first reproduce known staircase-style certificates only
after they are implemented inside the allowed code path:
\[
        \frac{14}{9}=1.555\ldots,
        \qquad
        \frac{20}{13}=1.538\ldots.
\]
Those values are targets, not current evidence from this repository.  The
discarded \texttt{square-stabbing-14-9} package should not be used as a
manuscript source.
...
\section{Exploratory appendix tasks}

Two exploratory tasks are implemented and have one audited overnight baseline.
They should be reported separately from the three main geometric objectives.

\subsection{Task A: rectangle graphs versus mixed square/segment graphs}

The run searched for a rectangle intersection graph with no bounded-grid
representation by a mixed family of squares and horizontal/vertical segments.
The audited baseline was:
\begin{itemize}
\item Slurm job \texttt{16501338};
\item run root \texttt{runs/explore\_overnight\_20260704\_051618};
\item six graph-separation rows;
\item all rows completed with exit code \texttt{0:0};
\item best exact score $0.0$;
\item best pressure/search score $1.3845054945054946$ from
\texttt{graph\_g3\_n7\_motif}.
\end{itemize}

Interpretation: the current stress objective finds difficult-looking
rectangle graphs, but the bounded mixed-representation verifier did not certify
a separation witness.  The next useful change is a stronger
non-representability verifier or a smaller structured graph family with more
informative unsat cores.

\subsection{Task B: halfplane epsilon-net lower-bound rediscovery}

The run searched for finite point sets and thresholds where every tested
$k$-point net misses a heavy halfplane.  The audited baseline was:
\begin{itemize}
\item six epsilon-net rows;
\item all rows completed with exit code \texttt{0:0};
\item best exact value $1.4545454545454546$ from
\texttt{eps\_n11\_t4\_k3};
\item \texttt{eps\_n12\_t5\_k3} reached the 8-hour budget without a positive
exact certificate.
\end{itemize}

The repository snapshot is kept at
\texttt{docs/assets/exploratory\_overnight\_20260704\_051618/}.  It contains
the matrix, summaries, best certificates, renderings, and a compact final
audit JSON, but not the large event streams.
...
\midrule
Unit-square stabbing & Staircase interval certificate and product lift & $\tau/\tau^*$ & independently reimplemented staircase target; current allowed baseline is $3/2$ \\
...
\item Mixing epsilon-net and graph-representability exploratory rows into the
main three-problem table.  They belong in a separate appendix unless the paper
