...
MATRIX="${1:-runs/control_matrix.jsonl}"
OUT_ROOT="${2:-runs/full_controls}"
...
python3 -m multilevel.cli report --summary "$OUT_ROOT/summary.csv" --out-dir "$OUT_ROOT/report"
python3 -m multilevel.cli audit --summary "$OUT_ROOT/summary.csv" --out "$OUT_ROOT/audit/audit.json" --csv "$OUT_ROOT/audit/audit.csv"
