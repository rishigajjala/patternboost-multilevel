...
ssh "$REMOTE" "bash -s" -- "$REMOTE_DIR" "$SHA" <<'REMOTE_SETUP'
set -euo pipefail
REMOTE_DIR="$1"
SHA="$2"
...
TMP_DIR="$(ssh "$REMOTE" "bash -s" -- "$REMOTE_DIR" <<'REMOTE_TMP'
set -euo pipefail
REMOTE_DIR="$1"
...
git archive --format=tar HEAD | ssh "$REMOTE" "tar -xf - -C '$TMP_DIR'"

ssh "$REMOTE" "bash -s" -- "$REMOTE_DIR" "$TMP_DIR" "$SHA" <<'REMOTE_FINALIZE'
set -euo pipefail
REMOTE_DIR="$1"
TMP_DIR="$2"
SHA="$3"
